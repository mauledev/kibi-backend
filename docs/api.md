...
POST   /staff/auth/policy/accept                            Accept the Responsible Use Policy (lifts the policy gate)
...
GET    /staff/superadmin/approvals                          List superadmin creation requests (dual-control queue)
POST   /staff/superadmin/approvals                          Propose creating a superadmin
GET    /staff/superadmin/approvals/{uuid}                   Get a single approval request
POST   /staff/superadmin/approvals/{uuid}/approve           Approve with a fresh TOTP — creates the superadmin
POST   /staff/superadmin/approvals/{uuid}/reject            Reject a pending request with a reason
```
