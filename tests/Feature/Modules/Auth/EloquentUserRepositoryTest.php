...
describe('EloquentUserRepository', function () {
    beforeEach(function () {
        ...
        app()->instance(TenantContext::class, new TenantContext(tenantId: $this->tenantA->id));
    ...
    describe('findByEmail', function () {
        it('returns user when email belongs to current tenant', function () {
            User::factory()->for($this->tenantA)->create(['email' => 'mine@test.com']);
        ...
        it('returns null when email belongs to a different tenant', function () {
            User::factory()->for($this->tenantB)->create(['email' => 'theirs@test.com']);
    ...
    describe('findById', function () {
        ...
        it('returns null when user belongs to a different tenant', function () {
            $user = User::factory()->for($this->tenantB)->create();
    ...
    describe('findByGoogleId', function () {
        it('returns user when google_id matches current tenant', function () {
            User::factory()->for($this->tenantA)->create(['google_id' => 'g-abc', 'email' => 'google@test.com']);
        ...
        it('returns null when google_id belongs to a different tenant', function () {
            User::factory()->for($this->tenantB)->create(['google_id' => 'g-xyz', 'email' => 'other@test.com']);
    ...
    describe('findByMicrosoftId', function () {
        it('returns user when microsoft_id matches current tenant', function () {
            User::factory()->for($this->tenantA)->create(['microsoft_id' => 'ms-123', 'email' => 'ms@test.com']);
        ...
        it('returns null when microsoft_id does not exist in current tenant', function () {
            User::factory()->for($this->tenantB)->create(['microsoft_id' => 'ms-456', 'email' => 'ms2@test.com']);
    ...
    describe('save', function () {
        it('creates the user record in the database', function () {
            // The Eloquent User model has uuid in $fillable, so we can pass it to save().
            // The toDomain() mapping requires a non-null uuid — ensure the factory sets one.
            $user = User::factory()->for($this->tenantA)->create(['email' => 'factory-created@test.com']);
    ...
    describe('update', function () {
        it('updates mutable fields and returns updated entity', function () {
            $user = User::factory()->for($this->tenantA)->create(['full_name' => 'Old Name']);
